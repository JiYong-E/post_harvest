...
def combined_browning_mask(
    ...
    browning_mask_large = final_mask_change  # 시각화용 이름
    ...
    browning_pixels = np.count_nonzero(final_mask)
    total_melon_pixels = np.count_nonzero(mask_alpha)
    browning_ratio = (
        browning_pixels / total_melon_pixels if total_melon_pixels > 0 else 0
    )

    fig, axes = plt.subplots(1, 2, figsize=(12, 6))

    fig.suptitle(f"Browning ratio: {browning_ratio:.4f}", fontsize=16, fontweight='bold')
    ...
    return final_mask, browning_mask_large, browning_mask_kmeans, mask_melon, mask_alpha

if __name__ == "__main__":
    # 본인 경로에 맞게 IMAGE_PATH 변경
    IMAGE_PATH = r"D:\2025년도\수업\2학기\스마트팜창의설계\data\images\20240404_103854_cutout_1.webp"

    final_mask, browning_mask_large, browning_mask_kmeans, mask_melon, mask_alpha = (
        combined_browning_mask(
            IMAGE_PATH,
            fixed_size=(800, 800),
            K_brown=5,
        )
